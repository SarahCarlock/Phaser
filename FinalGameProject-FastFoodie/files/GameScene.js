...
class GameScene extends Phaser.Scene {
  ...
  update() {
    ...
    /* KEYBOARD KEYS for food selection */
  ...
  placeFood(food, fullnessValue) {
    if (
      ...
      // Change color of hunger blocks to show what's left
      for (let i = 0; i < gameState.currentMeal.fullnessValue; i++) {
        if (i < gameState.currentCustomer.fullnessCapacity) {
          if (
            ...
            // If exactly full, box will be green
            ...
            // Otherwise, slight color change, and still hungry
            gameState.currentCustomer.fullnessMeterBlocks[i].setFillStyle(
              0xfffa81
            );
          }
  ...
  // Move the line of customers
  moveCustomerLine() {
    // Current customer is done and full
