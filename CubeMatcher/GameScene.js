...
class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    score = 0;
    let scoreText = this.add.text(15, 500, `Score: ${score}`, {
      fontSize: "25px",
      fill: "#fff",
    });
    /*
    // Add to previous scores array
    ...
    let previousScoreText = this.add.text(15, 550, `Previous Scores:`, {
      fontSize: "15px",
      fill: "#fff",
    });
    let previousScoreList = this.add.text(15, 575, `${previousScores}`, {
      fontSize: "15px",
      fill: "#fff",
    });
    */
    ...
    // Listener for clicks on cubes, aka Event Handler
...
const removeCols = () => {
  ...
  const emptyCols = board.map((col, i) => {
    // Check if every cube in the column has been removed
    const isEmpty = col.every(cube => cube.removed);
    // If the column is empty, return its index. Otherwise return false
...
const remainingMoves = () => {
  ...
  return board.some(doesColumnContainValidMoves);
};
